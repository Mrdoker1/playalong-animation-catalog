...
`;
let banner = null;
function frameBanner(slot, translateY, scale, opacity, sweepPos) {
    slot.innerHTML = `
    <div style="
      position:relative;
...
export const Feedback = {
    ...
    title: 'Feedback',
    game: 'Family Feud',
    description: 'When the round wraps up, a Round-Complete banner drops in from above with a back-ease overshoot. A soft light band sweeps left-to-right across the banner once, signalling "phase complete, moving on". A slow brightness pulse keeps the band alive afterwards.',
    ...
    play({ duration, easing }) {
        ...
        const sweep = banner.querySelector('.preview-ff-banner__sweep');
        if (sweep) {
            const clone = sweep.cloneNode(true);
            sweep.replaceWith(clone);
        }
    },
    snippets({ duration, easing }) {
