...
function frameTimer(slot: HTMLElement, opts: { label: string; fill: number; critical: boolean; timeUp?: boolean }) {
  if (opts.timeUp) {
    slot.innerHTML = `
      <div style="
...
export const Timer: AnimationModule = {
  id: 'cs-timer',
  title: 'Timer',
  game: 'Card Sharks',
  description:
    'Countdown bar that depletes linearly from full to empty over the question window. The blue gradient switches to red once the remaining time crosses a "critical" threshold (last 25%).',
  ...
  render(stage) {
    ...
    return () => {
      stage.innerHTML = '';
      bar = null; barTrack = null; value = null;
      if (tickInterval !== undefined) clearInterval(tickInterval);
    };
  },

  play({ duration }) {
    if (!bar || !value || !barTrack) return;
    if (tickInterval !== undefined) clearInterval(tickInterval);
    ...
    tickInterval = window.setInterval(() => {
      ...
      if (remaining <= 0) {
        ...
        if (barTrack) barTrack.classList.add('is-hidden');
      ...
      if (bar && remaining < duration * 0.25) bar.classList.add('is-critical');
    }, 100);
  },
