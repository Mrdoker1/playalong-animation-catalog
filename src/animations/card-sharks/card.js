...
let card = null;
let cardIdx = 0;
function frameCard(slot, tx, ty, rot, scale, opacity) {
    slot.innerHTML = `
    <div style="
...
export const Card = {
    ...
    game: 'Card Sharks',
    description: 'When a new question shows up, the next playing card flies in from off-screen top-left, rotating into place with a slight overshoot. The card lands flat and stays put until the next question.',
    ...
    play({ duration }) {
        ...
        const rank = card.querySelector('.preview-cs-card__rank');
        const suit = card.querySelector('.preview-cs-card__suit');
        if (rank)
            rank.textContent = face[2];
        if (suit)
            suit.textContent = face[0];
