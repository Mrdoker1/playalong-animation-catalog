...
function apply(btn, side, state) {
    ...
    if (label) {
        if (state === 'correct')
            ...
            label.textContent = side === 'up' ? 'Higher' : 'Lower';
    }
}
function frameBtn(label, state, dim) {
    const grads = {
        idle: '#202228',
...
function frameController(slot, higher, lower) {
    slot.innerHTML = `
...
export const Controller = {
    ...
    game: 'Card Sharks',
    description: 'Higher / Lower picker buttons. The player locks one option (blue gradient), then on reveal the correct option turns green ("Correct!") and the wrong pick turns red ("Incorrect!"). The unpicked option dims to 35 % once the round resolves.',
    ...
    `;
        ...
        [higherBtn, lowerBtn] = Array.from(stage.querySelectorAll('.preview-hl-btn'));
    ...
    play({ duration }) {
        ...
        if (higherBtn)
            higherBtn.style.transition = `background ${duration}ms ease, border-color ${duration}ms ease, opacity ${duration}ms ease`;
        if (lowerBtn)
            lowerBtn.style.transition = `background ${duration}ms ease, border-color ${duration}ms ease, opacity ${duration}ms ease`;
