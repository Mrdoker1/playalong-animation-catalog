import type { AnimationModule, AnimationParams, PlatformId } from '../types';
import { EASING_PRESETS, type EasingPreset, bezierToCss } from '../lib/easings';
...
export function createAnimationCard(anim: AnimationModule): HTMLElement {
  ...
  const slowBtn = root.querySelector<HTMLButtonElement>('[data-role=slow]')!;
  const resetBtn = root.querySelector<HTMLButtonElement>('[data-role=reset]')!;
  const copySnippetBtn = root.querySelector<HTMLButtonElement>('[data-role=copy-snippet]')!;
  const copyPmBtn = root.querySelector<HTMLButtonElement>('[data-role=copy-pm]')!;
  const durationInput = root.querySelector<HTMLInputElement>('[data-role=duration]')!;
  const durationValue = root.querySelector<HTMLElement>('[data-role=duration-value]')!;
  const easingPreset = root.querySelector<HTMLSelectElement>('[data-role=easing-preset]')!;
  const easingDesc = root.querySelector<HTMLElement>('[data-role=easing-desc]')!;
  const bezierInputs = [0, 1, 2, 3].map(
    (i) => root.querySelector<HTMLInputElement>(`[data-role=bezier-${i}]`)!,
  );
  const snippetOutput = root.querySelector<HTMLElement>('[data-role=snippet-output]')!;
  const pmOutput = root.querySelector<HTMLElement>('[data-role=pm-output]')!;
  const tabButtons = Array.from(root.querySelectorAll<HTMLButtonElement>('.tab'));
  ...
  function selectPreset(p: EasingPreset) {
    state.easing = { ...p };
    easingDesc.textContent = p.description;
    syncBezierInputs();
    refreshOutputs();
    if (!anim.tbd) anim.play(state);
  }

  function applyBezierFromInputs() {
    const vals = bezierInputs.map((inp) => Number(inp.value)) as [number, number, number, number];
  ...
  if (anim.frames && anim.frames.length > 0) {
    anim.frames.forEach((f, idx) => {
      const slot = root.querySelector<HTMLElement>(`[data-frame-idx="${idx}"]`);
      if (slot) f.render(slot);
    });
  }

  // ---- Events ----------------------------------------------------------------
  playBtn.addEventListener('click', () => { if (!anim.tbd) anim.play(state); });
  ...
  tabButtons.forEach((b) => {
    b.addEventListener('click', () => {
      activePlatform = b.dataset.platform as PlatformId;
      refreshOutputs();
    });
  });
  ...
  refreshOutputs();
  if (!anim.tbd) requestAnimationFrame(() => anim.play(state));

  (root as HTMLElement & { __destroy?: () => void }).__destroy = () => { cleanup(); };
